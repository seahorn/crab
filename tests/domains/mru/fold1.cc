...
// vars: Dom -> vector(Var)
// extract a set of variables in the environment of abstractr domain
template <class Dom> variable_vector_t vars(Dom const &dom) {
  variable_map_t vars;
...
// perform a similiar work for cache.lines.meet(cache.regs)
template <class Dom1, class Dom2> Dom1 meet(Dom1 dom1, Dom2 const &dom2) {
  auto csts = dom2.to_linear_constraint_system();
  dom1 += csts;
  return dom1;
}

// perform a set minus like operation for variable vectors
...
int main(int argc, char **argv) {
  ...
  { // fold op
    ...
    // strong update
    cache.cache_lines.assign(cache_line1, z_number(3));

    // Simulate ref_store(ref + 4, rgn2, n5_32)
    // strong update
    cache.cache_lines.assign(cache_line2, z_number(5));
    /*
      cache: ( { C_1 = 3; C_2 = 5 },
      top,
      { C_base == ref.base },
