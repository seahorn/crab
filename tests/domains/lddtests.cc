...
void dump(LddNodePtr val,  LddManager *man) {
  ...
  if (val.get() == Ldd_GetTrue(man)) crab::outs() << "true\n";
  else if (val.get() == Ldd_GetFalse(man)) crab::outs() << "false\n";	
  else Ldd_PrintMinterm(man,val.get());
  Cudd_SetStdout(cudd,fp);      
}

LddNodePtr top(LddManager *man) 
{
  return lddPtr(man, Ldd_GetTrue(man));
}

LddNodePtr bot(LddManager *man)
{
  return lddPtr(man, Ldd_GetFalse(man));
}

bool isBot(LddManager * man, LddNodePtr v)
{
  return &*v == Ldd_GetFalse(man);
}

bool isTop(LddManager * man, LddNodePtr v)
{
  return &*v == Ldd_GetTrue(man);
}
...
linterm_t termForVal(LddManager* man, varname_t v, bool neg = false) 
{
  ...
  int sgn = neg ? -1 : 1;
  linterm_t term = Ldd_GetTheory(man)->create_linterm_sparse_si(&varId, &sgn, 1);
  return term; 
}
...
LddNodePtr assign(LddManager* man, LddNodePtr n, varname_t v, interval_t ival) {
  ...
  if (kmin) Ldd_GetTheory(man)->destroy_cst(kmin);
  if (kmax) Ldd_GetTheory(man)->destroy_cst(kmax);
...
LddNodePtr apply(LddManager* man,
                  const LddNodePtr n, 
                  varname_t v, varname_t u,
                  number_t a, number_t k)
{
  if (isTop(man, n) || isBot(man, n)) return n;
...
/** v := u */
LddNodePtr assign(LddManager* man, const LddNodePtr n, 
                   varname_t v, varname_t u)
{
