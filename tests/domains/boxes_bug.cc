...
int main (int argc, char** argv ) {
  ...
  variable_factory_t vfac;
  {
    ...
    csts1 += (q_lin_t (vfac ["x"]) >= q_number(2));
    csts1 += (q_lin_t (vfac ["y"]) == q_number(2));
    q_boxes_domain_t inv1_c1 = q_boxes_domain_t::top ();
    inv1_c1 += csts1;

    csts2 += (q_lin_t (vfac ["x"]) == q_number(1));
    csts2 += (q_lin_t (vfac ["y"]) == q_number(1));
    q_boxes_domain_t inv1_c2 = q_boxes_domain_t::top ();
    inv1_c2 += csts2;

    csts3 += (q_lin_t (vfac ["x"]) == q_number(1));
    csts3 += (q_lin_t (vfac ["y"]) == q_number(0));
    ...
    // build first widening argument 
    q_lin_cst_sys_t csts4, csts5, csts6, csts7, csts8, csts9, csts10, csts11, csts12 ;
    
    csts4 += (q_lin_t (vfac ["x"]) > q_number(3));
    csts4 += (q_lin_t (vfac ["y"]) == q_number(3));
    q_boxes_domain_t inv2_c1 = q_boxes_domain_t::top ();
    inv2_c1 += csts4;

    csts5 += (q_lin_t (vfac ["x"]) >= q_number(2));
    csts5 += (q_lin_t (vfac ["x"]) <= q_number(3));    
    csts5 += (q_lin_t (vfac ["y"]) == q_number(3));
    q_boxes_domain_t inv2_c2 = q_boxes_domain_t::top ();
    inv2_c2 += csts5;

    csts6 += (q_lin_t (vfac ["x"]) >= q_number(2));
    csts6 += (q_lin_t (vfac ["x"]) <= q_number(3));    
    csts6 += (q_lin_t (vfac ["y"]) == q_number(2));
    q_boxes_domain_t inv2_c3 = q_boxes_domain_t::top ();
    inv2_c3 += csts6;

    csts7 += (q_lin_t (vfac ["x"]) >= q_number(2));
    csts7 += (q_lin_t (vfac ["x"]) <= q_number(3));    
    csts7 += (q_lin_t (vfac ["y"]) == q_number(1));
    q_boxes_domain_t inv2_c4 = q_boxes_domain_t::top ();
    inv2_c4 += csts7;

    csts8 += (q_lin_t (vfac ["x"]) > q_number(1));
    csts8 += (q_lin_t (vfac ["x"]) < q_number(2));    
    csts8 += (q_lin_t (vfac ["y"]) == q_number(2));
    q_boxes_domain_t inv2_c5 = q_boxes_domain_t::top ();
    inv2_c5 += csts8;

    csts9 += (q_lin_t (vfac ["x"]) > q_number(1));
    csts9 += (q_lin_t (vfac ["x"]) < q_number(2));    
    csts9 += (q_lin_t (vfac ["y"]) == q_number(1));
    q_boxes_domain_t inv2_c6 = q_boxes_domain_t::top ();
    inv2_c6 += csts9;

    csts10 += (q_lin_t (vfac ["x"]) == q_number(1));
    csts10 += (q_lin_t (vfac ["y"]) == q_number(2));
    q_boxes_domain_t inv2_c7 = q_boxes_domain_t::top ();
    inv2_c7 += csts10;

    csts11 += (q_lin_t (vfac ["x"]) == q_number(1));
    csts11 += (q_lin_t (vfac ["y"]) == q_number(1));
    q_boxes_domain_t inv2_c8 = q_boxes_domain_t::top ();
    inv2_c8 += csts11;

    csts12 += (q_lin_t (vfac ["x"]) == q_number(1));
    csts12 += (q_lin_t (vfac ["y"]) == q_number(0));
    ...
    crab::outs () << "[A] and [B] should be the same if CONVEX distributes over WIDENING \n";
