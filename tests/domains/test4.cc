...
int main (int argc, char** argv) {
  ...
  run<z_interval_domain_t>(cfg,cfg->entry(),true,1,2,20,stats_enabled);
  run<z_dbm_domain_t>(cfg,cfg->entry(),true,1,2,20,stats_enabled);
  run<z_sdbm_domain_t>(cfg,cfg->entry(),true,1,2,20,stats_enabled);
  run<z_ric_domain_t>(cfg,cfg->entry(),true,1,2,20,stats_enabled);
  run<z_term_domain_t>(cfg,cfg->entry(),true,1,2,20,stats_enabled);
  run<z_dis_interval_domain_t>(cfg,cfg->entry(),true,1,2,20,stats_enabled);
  delete cfg;
  return 0;
}
