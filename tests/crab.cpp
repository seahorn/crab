...
void intra_run_impl(const CFG &cfg, crab::cfg_impl::basic_block_label_t entry,
  ...
  // this okay because cfg_ref won't be mutated.
  auto cfg_ref = cfg_ref_t(*(const_cast<CFG*>(&cfg)));

  crab::analyzer::live_and_dead_analysis<cfg_ref_t> live(cfg_ref);
  if (run_liveness) {
    live.exec();
  }
  // Run fixpoint
  ...
  IntraFwdAnalyzer a(cfg_ref, absval_fac, (run_liveness) ? &live : nullptr, fixpo_params);

  assumption_map_t assumptions;
  a.run(entry, init, assumptions);

  CRAB_LOG("crab-tests-print-invariants", print_invariants = true);
  ...
  if (enable_checker) {
    ...
    using assert_checker_t = crab::checker::assert_property_checker<IntraFwdAnalyzer>;
    const int verbose = 0;
...
void z_intra_run(const crab::cfg_impl::z_cfg_t &cfg,
  ...
  using intra_fwd_analyzer_t = intra_fwd_analyzer<crab::cfg_impl::z_cfg_ref_t, Dom>;
...
template <typename Dom>
void q_intra_run(const crab::cfg_impl::q_cfg_t &cfg,
                 crab::cfg_impl::basic_block_label_t entry, Dom init,
                 bool run_liveness, unsigned widening, unsigned narrowing,
                 unsigned jump_set_size, bool enable_stats,
                 bool enable_checker, bool print_invariants) {
  using namespace crab::analyzer;
  using intra_fwd_analyzer_t = intra_fwd_analyzer<crab::cfg_impl::q_cfg_ref_t, Dom>;
