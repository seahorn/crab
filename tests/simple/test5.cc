...
int main (int argc, char** argv )
{
...

  {
    ...
    cout << "Invariants using " << sdbm_domain_t::getDomainName () << "\n";
    for (auto &b : cfg)
    {
      auto inv = a [b.label ()];
      std::cout << get_label_str (b.label ()) << "=" << inv << "\n";
  ...
  {
    NumFwdAnalyzer <cfg_t, term_dis_int_t,VariableFactory>::type a (cfg,vfac,&live);
    a.Run (term_dis_int_t::top ());
    cout << "Invariants using " << term_dis_int_t::getDomainName () << "\n";
    for (auto &b : cfg)
    {
      auto inv = a [b.label ()];
      std::cout << get_label_str (b.label ()) << "=" << inv << "\n";
  ...
  {
    NumFwdAnalyzer <cfg_t, num_domain_t,VariableFactory>::type a (cfg,vfac,&live, 1, 2, 20);
    a.Run (num_domain_t::top ());
    cout << "Invariants using " << num_domain_t::getDomainName () << "\n";
    for (auto &b : cfg) 
    {
      auto inv = a [b.label ()];
      std::cout << get_label_str (b.label ()) << "=" << inv << "\n";
