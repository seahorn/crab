...
wrapint::wrapint(uint64_t n, bitwidth_t width)
  : _n(n), _width(width), _mod(0) {
...
wrapint::bitwidth_t wrapint::get_bitwidth() const {
  return _width;
}
...
// return 0000....0
wrapint wrapint::get_unsigned_min(bitwidth_t w) {
  return wrapint(0, w);
}
...
uint64_t wrapint::get_uint64_t() const {
  return _n;
}
...
void wrapint::write(crab::crab_os& o) const {
  o << get_unsigned_str();
}
