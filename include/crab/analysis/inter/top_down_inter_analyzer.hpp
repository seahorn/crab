  ...
  while (index < param_lst_sz) {
    ...
    for (auto const &intrinsic_stmt : intrinsic_stmts) {
      ...
      for (unsigned j = 0; j < intrinsic_arg_sz; j++) {
        ...
        if (index < param_lst_sz &&
            args[j].get_variable() == tgt_params[index]) {
...
// performed before the inter-procedural analysis.
// input: call graph \p g, global_context \ctx
// a. iterate all cfg inside wto of cfgs
// b. for each cfg, visit callsite statement
    ...
    for (auto const &bb : caller_cfg) {
      for (auto const &stmt : bb) {
        if (stmt.is_callsite()) {
          ...
          // get callee's cfg
          if (cg.has_callee(cs)) {
            CRAB_LOG("inter-group",
                     errs() << "Callee: " << cg.get_callee(cs).name();
                     errs() << "\n";);
            ...
            callsite_info_t callsite_info{
                cs.get_func_name(),  cs.get_args(),
                cs.get_lhs(),        fdecl.get_inputs(),
                fdecl.get_outputs(), std::move(actual_cls),
                std::move(lhs_cls),  std::move(formal_cls),
                std::move(ret_cls)};
  ...
  void analyze_callee(const callsite_t &cs, cg_node_t callee_cg_node) {
	...
	if (m_ctx.find_call_stack(callee_cg_node)) {
	  ...
	  // foo since its analysis is not completed yet.
          CRAB_VERBOSE_IF(1, get_msg_stream() << callee_at_exit << "\n";);
