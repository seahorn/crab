...
private:
  ...
  static_assert(std::is_same<basic_block_label_t, typename InvariantMap::key_type>::value,
		"The key of an invariant map should be basic block label");
  static_assert(std::is_same<abs_dom_t, typename InvariantMap::mapped_type>::value,
...
public:
  ...
  fdecl_t get_fdecl() const {
    return m_fdecl;
  }

  // return I from the pair (I,O)  
  abs_dom_t get_input() const {
    return m_input;    
  }

  // return O from the pair (I,O)
  abs_dom_t get_output() const {
    return m_output;        
  }
  ...
  std::unique_ptr<calling_context_t> join_with(calling_context_t& other) {
    if (!(get_fdecl() == other.get_fdecl())) {
      CRAB_ERROR("Cannot join calling contexts because they are from different functions");
    }
    
    return std::unique_ptr<calling_context_t>(
	   new calling_context_t(m_fdecl,
				 m_input  | other.get_input(),
				 m_output | other.get_output(), 
...
class context_sensitivity_policy {
protected:
...
class default_context_sensitivity_policy:
  ...
  using context_sensitivity_policy_t = context_sensitivity_policy<CallingContext>;
  using abs_dom_t = typename CallingContext::abs_dom_t;
  
public:

  using calling_context_ptr = typename context_sensitivity_policy_t::calling_context_ptr;  
  ...
  virtual void add(calling_context_ptr_deque& ccs, calling_context_ptr cc) override {
    ...
    CRAB_LOG("inter3",
	     crab::outs() << "All calling contexts before\n";
	...
	// calling context
	if (!((*it)->get_input() <= joined_input && (*it)->get_output() <= joined_output)) {
	  new_ccs.push_back(std::move(*it));
	}
      }
    ...
    CRAB_LOG("inter3",
	     crab::outs() << "All calling contexts after\n";
	     for(unsigned i=0,e=ccs.size();i<e;++i) {
	       ccs[i]->dump();
	     });
  }
...
public:
  ...
  using context_sensitivity_policy_t = context_sensitivity_policy<calling_context_t>;
...
private:
  ...
  void join_with(global_invariant_map_t& global_table, cfg_t cfg, invariant_map_t& other) {
...
public:
  ...
  global_context(const liveness_map_t* live_map,
    ...
    : m_live_map(live_map) 
    , m_wto_cfg_map(wto_cfg_map)
    , m_cs_policy(new default_context_sensitivity_policy_t(max_call_contexts))
    , m_enable_checker(enable_checker)
    , m_checker_verbosity(checker_verbosity)
    , m_is_checking_phase(false)
  ...
  const std::set<CallGraphNode>& get_widening_set() const { return m_widening_set;}
  ...
  context_sensitivity_policy_t& get_context_sensitivity_policy(){ return *m_cs_policy;}
  ...
  const calling_context_table_t& get_calling_context_table() const { return m_cc_table;}
  ...
  global_invariant_map_t& get_global_pre_invariants() { return m_pre_invariants;}

  const global_invariant_map_t& get_global_pre_invariants() const { return m_pre_invariants;}

  global_invariant_map_t& get_global_post_invariants() { return m_post_invariants;}

  const global_invariant_map_t& get_global_post_invariants() const { return m_post_invariants;}
...
get_inter_analysis(CallGraphNode cg_node,
  ...
  using assertion_checker_t = checker::assert_property_checker<IntraCallSemAnalyzer>;
  using checks_db_t = checker::checks_db;
  ...
  ctx.join_invariants_with(cg_node,
			   analyzer->get_pre_invariants(),
  ...
  if (ctx.run_checker()) {
    ...
    typename intra_checker_t::prop_checker_ptr
      p(new assertion_checker_t(ctx.get_checker_verbosity()));
...
class top_down_inter_transformer final:
  ...
  using abs_transform_api_t = typename intra_abs_transformer_t::abs_transform_api_t;
...
public:
  ...
  using calling_context_collection_t = typename global_context_t::calling_context_collection_t;
  using calling_context_table_t = typename global_context_t::calling_context_table_t;
...
private:
  ...
  static void get_fdecl_parameters(fdecl_t& fdecl, std::vector<variable_t>& out) {
    out.reserve(fdecl.get_num_inputs() + fdecl.get_num_outputs());
    out.insert(out.end(), fdecl.get_inputs().begin(), fdecl.get_inputs().end());
    out.insert(out.end(), fdecl.get_outputs().begin(), fdecl.get_outputs().end());
  }
   ...
   **/
  static AbsDom get_caller_continuation(callsite_t& cs, fdecl_t& fdecl,
					AbsDom caller_dom,
  ...
  void analyze_callee(callsite_t& cs, cg_node_t callee_cg_node) {
    ...
    if (::crab::CrabSanityCheckFlag) {
      ...
      if (!(pre_bot || !post_bot)) {
	CRAB_ERROR("Invariant became bottom after propagating from actual to formals ",
		   cs);
      }
    }
	...
	CRAB_LOG("inter",
      ...
      calling_context_ptr cc(
	  new calling_context_t(fdecl, callee_entry, callee_exit,
				m_ctx.minimize_invariants(),
    ...
    AbsDom caller_cont_dom = get_caller_continuation(cs, fdecl, caller_dom,
						     callee_exit_vars, callee_exit);

    if (::crab::CrabSanityCheckFlag) {
      bool post_bot = caller_cont_dom.is_bottom();
      if (!(pre_bot || !post_bot)) {
	CRAB_ERROR("Invariant became bottom after propagating from actuals to formals ",
		   cs);
      }
    }
    
    CRAB_LOG("inter",
	     crab::outs() << "[INTER] Continuation for \"" << cs << "\"="
	                  << caller_cont_dom << "\n";);
...
public:
  ...
  const global_context_t& get_context() const {
    return m_ctx;
  }

  global_context_t& get_context() {
    return m_ctx;
  }
  ...
  virtual void exec(callsite_t& cs) override {
    ...
    if (std::find(call_stack.begin(), call_stack.end(),
		  callee_cg_node) == call_stack.end()) {
      if (m_ctx.get_widening_set().count(callee_cg_node) > 0) {
...
struct top_down_inter_analyzer_parameters {
  using cg_node_t = typename CallGraph::node_t;
  using cfg_t = typename cg_node_t::cfg_t;
  ...
  top_down_inter_analyzer_parameters()
    : run_checker(true)
    , checker_verbosity(0)
    , minimize_invariants(true)
    , max_call_contexts(UINT_MAX)
    , widening_delay(2)
    , descending_iters(2 /*UINT_MAX*/)
    , thresholds_size(20 /*0*/)
    , live_map(nullptr)
    , wto_map(nullptr) {}
...
private:
  ...
  using invariant_map_t = typename intra_analyzer_with_call_semantics_t::invariant_map_t;
  
  // global context stuff
  using global_context_t =
    top_down_inter_impl::global_context<typename CallGraph::node_t, abs_dom_t, invariant_map_t>;
  using calling_context_t = typename global_context_t::calling_context_t;  
  using calling_context_ptr = typename global_context_t::calling_context_ptr;
  using calling_context_table_t = typename global_context_t::calling_context_table_t;
  using calling_context_collection_t=typename global_context_t::calling_context_collection_t;
  using global_invariant_map_t = typename global_context_t::global_invariant_map_t;

  // detect widening points in the call graph
  using wto_cg_t = ikos::wto<CallGraph>;
  struct find_widening_set_visitor: public ikos::wto_component_visitor<CallGraph> {
...
public:
  ...
  top_down_inter_analyzer(CallGraph cg, const params_t& params = params_t())
    : m_cg(cg)
    , m_ctx(params.live_map, params.wto_map, params.run_checker, params.checker_verbosity,
	    params.minimize_invariants, params.max_call_contexts,
	    params.widening_delay, params.descending_iters, params.thresholds_size) {
  ...
  // Return the *context-insensitive* invariants that hold at the entry of b in cfg.	
  AbsDom get_pre(const cfg_t &cfg, basic_block_label_t bb) const {
    return get_invariant(m_ctx.get_global_pre_invariants(), cfg, bb);
  }
      
  // Return the *context-insensitive* invariants that hold at the exit of b in cfg.
  ...
  void clear() {
    CRAB_WARN("clear operation of the inter-procedural analysis not implemented yet");
  }

  const checks_db_t get_all_checks() const {
    return m_ctx.get_checks_db();
  }

  checks_db_t get_all_checks() {
    return m_ctx.get_checks_db();
  }
