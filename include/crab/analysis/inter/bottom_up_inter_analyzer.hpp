...
namespace crab {
namespace analyzer {
namespace bottom_up_inter_impl {  
...
/**
 * Store the calling contexts of each function. The calling context
 * is the abstract state that holds when the function is called.
 **/
...
private:
  ...
  // Assume context-insensitive analysis so it will merge all calling
  // contexts using abstract domain's join keeping a single calling
  ...
  void insert_helper(callsite_or_fdecl_t key, AbsDomain &&inv) {
    ...
    if (it != m_call_table.end()) {
      ...
      m_call_table.insert({key, std::move(inv)});
    }
  }

public:
  call_ctx_table(AbsDomain top) : m_top_fac(top.make_top()) {}
  ...
  void insert(const callsite_t &cs, AbsDomain &&inv) {
    insert_helper(&cs, std::move(inv));
  }

  void insert(const fdecl_t &d, AbsDomain &&inv) {
    insert_helper(&d, std::move(inv));
  }

  const AbsDomain& get_call_ctx(const fdecl_t &d) const {
...
template <typename CFG, typename AbsDomain> class summary {
  ...
  // Summary defined in terms of m_fdecl's inputs and outputs
  abs_domain_t m_sum;
  // Copy of input function parameters using ghost variables
  std::vector<variable_t> m_ghost_inputs;
  // Copy of output function parameters using ghost variables
  std::vector<variable_t> m_ghost_outputs;
  // Summary defined in terms for m_ghost_inputs and m_ghost_outputs
  abs_domain_t m_ghost_sum;

  void compute_ghost_summary()  {
    std::vector<variable_t> from_vars, to_vars;
    from_vars.reserve(m_fdecl.get_inputs().size() + m_fdecl.get_outputs().size());
    from_vars.insert(from_vars.end(), m_fdecl.get_inputs().begin(), m_fdecl.get_inputs().end());
    from_vars.insert(from_vars.end(), m_fdecl.get_outputs().begin(), m_fdecl.get_outputs().end());
    to_vars.reserve(m_ghost_inputs.size() + m_ghost_outputs.size());
    to_vars.insert(to_vars.end(), m_ghost_inputs.begin(), m_ghost_inputs.end());
    to_vars.insert(to_vars.end(), m_ghost_outputs.begin(), m_ghost_outputs.end());
    m_ghost_sum.rename(from_vars, to_vars);
  }

public:
  summary(const fdecl_t &fdecl, abs_domain_t &&sum)
    : m_fdecl(fdecl), m_sum(std::move(sum)), m_ghost_sum(m_sum) {
    
    m_ghost_inputs.reserve(fdecl.get_num_inputs());
    m_ghost_outputs.reserve(fdecl.get_num_outputs());
    
    unsigned var_id = 0;
    for (auto const &v: m_fdecl.get_inputs()) {
      auto &vfac = const_cast<varname_t *>(&(v.name()))->get_var_factory();
      std::string arg_name;
      arg_name.reserve(5+2); // expect 99999 arguments as max + 1 ($) + 1
      arg_name.append("$");      
      arg_name.append(std::to_string(var_id));
      variable_t fresh_v(vfac.make_varname(std::move(arg_name)), v.get_type());
      m_ghost_inputs.push_back(fresh_v);
      var_id++;
    }
    
    for (auto const &v: m_fdecl.get_outputs()) {  
      auto &vfac = const_cast<varname_t *>(&(v.name()))->get_var_factory();
      std::string arg_name;
      arg_name.reserve(5+2); // expect 99999 arguments as max + 1 ($) + 1
      arg_name.append("$");      
      arg_name.append(std::to_string(var_id));
      variable_t fresh_v(vfac.make_varname(std::move(arg_name)), v.get_type());
      m_ghost_outputs.push_back(fresh_v);
      var_id++;
    }
    
    compute_ghost_summary();
  }

  // The summary defined over fdecl's variables
  const abs_domain_t& get_sum() const { return m_sum; }

  abs_domain_t make_top() const { return m_sum.make_top(); }

  // return the input variables of the summary
  const std::vector<variable_t> &get_inputs() const { return m_fdecl.get_inputs(); }

  // return the output variables of the summary
  const std::vector<variable_t> &get_outputs() const { return m_fdecl.get_outputs(); }

  // The summary defined over ghost variables. This avoids naming
  // clashes when summaries are used in the interprocedural analysis.
  const abs_domain_t& get_ghost_sum() const { return m_ghost_sum; }

  const std::vector<variable_t> &get_ghost_inputs() const {
    return m_ghost_inputs;
  }

  const std::vector<variable_t> &get_ghost_outputs() const {
    return m_ghost_outputs;
  }

  void write(crab_os &o) const {
    o << m_fdecl.get_func_name() << "(IN:{";
    for (unsigned i = 0; i < m_fdecl.get_num_inputs(); i++) {
      o << m_fdecl.get_inputs()[i] << ":" << m_fdecl.get_input_type(i);
      if (i != m_fdecl.get_num_inputs() - 1)
        o << ",";
    }
    o << "},OUT:{";
    for (unsigned i = 0; i < m_fdecl.get_num_outputs(); i++) {
      o << m_fdecl.get_outputs()[i] << ":" << m_fdecl.get_output_type(i);
...
public:
  ...
  void add(const fdecl_t &d, AbsDomain &&sum) { 
    summary_t sum_tuple(d, std::move(sum));
  ...
  // get the summary
  const summary_t& get(const callsite_t &cs) const {
    assert(has_summary(cs));
    auto it = m_sum_table.find(&cs);
    assert(it != m_sum_table.end());
    return (it->second);
  }

  const summary_t& get(const fdecl_t &d) const {
    assert(has_summary(d));
    auto it = m_sum_table.find(&d);
    assert(it != m_sum_table.end());
    return (it->second);
  }
...
public:
  ...
  static void reuse_summary(abs_dom_t &caller, const callsite_t &cs,
    ...
    auto const& inputs = summ.get_ghost_inputs();
    unsigned i = 0;
    // Propagating down
    ...
    auto const& sum_inv = summ.get_ghost_sum();
    ...
    // Propagate from the return values in the callee to the lhs
    // variables of the callsite in the caller.
    auto const &caller_vts = cs.get_lhs();
    auto const &callee_outs = summ.get_ghost_outputs();
    ...
    // Remove from caller only formal parameters so we can keep as
    // much context from the caller as possible
...
public:
  ...
  virtual void exec(callsite_t &cs) override {
    ...
    if (m_sum_tbl.has_summary(cs)) {
      ...
      // --- matching formal and actual parameters by propagating down
      unsigned i = 0;
      ...
      callee_ctx_inv.project(summ.get_inputs());
      // Store the callee context
      CRAB_LOG("inter", crab::outs() << "\t\tCallee context stored: "
                                     << callee_ctx_inv << "\n");

      m_call_tbl.insert(cs, std::move(callee_ctx_inv));
...
private:
  using summ_tbl_t = bottom_up_inter_impl::summary_table<cfg_t, BU_Dom>;
  using call_tbl_t = bottom_up_inter_impl::call_ctx_table<cfg_t, TD_Dom>;
  using cg_ref_t = crab::cg::call_graph_ref<cg_t>;
  using callsite_or_fdecl_t = crab::cfg::callsite_or_fdecl<cfg_t>;  
public:
  using bu_abs_tr_t = bottom_up_inter_impl::bu_summ_abs_transformer<summ_tbl_t>;
  using td_abs_tr_t =
      bottom_up_inter_impl::td_summ_abs_transformer<summ_tbl_t, call_tbl_t>;
  using bu_analyzer_t = analyzer_internal_impl::fwd_analyzer<cfg_t, bu_abs_tr_t>;
  using td_analyzer_t = analyzer_internal_impl::fwd_analyzer<cfg_t, td_abs_tr_t>;
  using summary_t = typename inter_analyzer_api<CallGraph,TD_Dom,BU_Dom>::summary_t;
  
  // for checkers
  using abs_dom_t = TD_Dom;
  using abs_tr_t = td_abs_tr_t;

private:
  using td_analyzer_ptr = std::unique_ptr<td_analyzer_t>;
  ...
  // Invariant map for external queries
  invariant_map_t m_inv_map;
  // Summary map for external queries
  mutable summary_map_t m_summary_map;
  
  // Summaries computed during the bottom-up phase
  summ_tbl_t m_summ_tbl;
  // Initial invariants for each function when analyzed during the
  // top-down phase.
...
public:
  bottom_up_inter_analyzer(CallGraph &cg,
    ...
    : m_cg(cg),
      m_td_top_fac(td_absval_fac.make_top()), m_bu_top_fac(bu_absval_fac.make_top()),
      m_live(params.live_map),
      m_call_tbl(m_td_top_fac),
      m_abs_tr(new abs_tr_t(m_td_top_fac, m_summ_tbl, m_call_tbl)) {
  ...
  void run(TD_Dom init) override {
	...
	BU_INTER_TIMER_STATS("Inter.TopDown");	
        ...
        td_analyzer_ptr a(new td_analyzer_t(cfg, &*m_abs_tr, m_td_top_fac, get_live(cfg), m_fixpo_params)); 
        a->run_forward(init);
    ...
    for (auto const &n : rev_order) {
      ...
      for (auto m : scc_mems) {
        ...
        if (fun_name != "main") {
          ...
          if (fdecl.get_outputs().empty()) {
	    BU_Dom top(m_bu_top_fac);
            m_summ_tbl.add(fdecl, std::move(top));  
    ...
    for (auto n :
	  ...
	  TD_Dom top(m_td_top_fac);
          m_call_tbl.insert(fdecl, std::move(top));
        }
        ...
        td_analyzer_ptr a(new td_analyzer_t(cfg, &*m_abs_tr, m_td_top_fac, get_live(cfg), 
					    m_fixpo_params));
  ...
  const summary_t& get_summary(const cfg_t &cfg) const override {
	...
	auto internal_summary =  m_summ_tbl.get(fdecl);
