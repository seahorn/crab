...
template <typename CFG> struct assert_wrapper {
  ...
  typedef std::unordered_map<basic_block_label_t,
                             std::vector<basic_block_label_t>>
      cdg_t;
...
public:
  ...
  typedef separate_killgen_domain<assert_wrapper_t, var_dom_t>
      separate_domain_t;

private:
  typedef killgen_operations_api<CFG, separate_domain_t>
  ...
  class transfer_function : public statement_visitor<N, V> {
    ...
    public:
      ...
      operator()(separate_domain_t inv) { // XXX: separate_domain_t cannot be
                                          // modified in-place
        typedef std::pair<typename separate_domain_t::key_type,
...
public:
  ...
  typedef typename assertion_crawler_operations<CFG>::separate_domain_t
      separate_domain_t;
  ...
  void get_assertions(
    ...
    if (it != m_map.end()) {
      if (!it->second.is_bottom()) {
        ...
        typename assertion_crawler_operations<CFG>::transfer_function vis(
            it->second, *(kv.first.cdg_ptr), *(kv.first.assert_map_ptr), bb);
        for (auto &s : boost::make_iterator_range(bb.rbegin(), bb.rend())) {
          // -- post-state
          // auto out = vis.inv ();
