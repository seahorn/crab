...
void dominator_tree(G g, typename G::node_t entry, Map &idom) {
  ...
  using time_map_t = boost::iterator_property_map<
      typename std::vector<vertices_size_type_t>::iterator, index_map_t>;
  using pred_map_t =
      boost::iterator_property_map<typename std::vector<node_t>::iterator,
                                   index_map_t>;
  ...
  std::vector<node_t> parent(num_vertices(g),
  ...
  std::vector<node_t> dom_tree_pred_vector(
      num_vertices(g), boost::graph_traits<G>::null_vertex());
  pred_map_t dom_tree_pred_map(
      make_iterator_property_map(dom_tree_pred_vector.begin(), index_map));

  boost::lengauer_tarjan_dominator_tree(g, entry, index_map, df_num_map,
                                        parent_map, vertices_by_df_num,
                                        dom_tree_pred_map);
  ...
  for (auto n : boost::make_iterator_range(vertices(g))) {
    for (auto e : boost::make_iterator_range(in_edges(n, g))) {
      ...
      while (runner != boost::graph_traits<G>::null_vertex() &&
        ...
        if (std::find(df[runner].begin(), df[runner].end(), n) ==
            df[runner].end())
...
template <typename G, typename VectorMap> void dominance(G g, VectorMap &fdf) {
