...
void rev_topo_sort(const G &g, std::vector<typename G::node_t> &res) {
  ...
  typedef std::unordered_map<typename G::node_t, boost::default_color_type>
      color_map_t;
...
template <class G> std::vector<typename G::node_t> weak_rev_topo_sort(G g) {
...
// XXX: related concept but this is not Bourdoncle's WTO
template <class G> std::vector<typename G::node_t> weak_topo_sort(G g) {
  std::vector<typename G::node_t> sccg_order;
