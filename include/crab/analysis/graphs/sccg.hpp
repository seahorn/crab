...
template <typename G> class scc_graph {
...
private:
  ...
  using scc_graph_t = boost::adjacency_list<
  ...
  using vertex_descriptor_t =
      typename boost::graph_traits<scc_graph_t>::vertex_descriptor;
  using edge_descriptor_t =
      typename boost::graph_traits<scc_graph_t>::edge_descriptor;
  using vertex_iterator =
      typename boost::graph_traits<scc_graph_t>::vertex_iterator;
  using out_edge_iterator =
      typename boost::graph_traits<scc_graph_t>::out_edge_iterator;
  using in_edge_iterator =
      typename boost::graph_traits<scc_graph_t>::in_edge_iterator;
  /// --- end internal representation of the scc_graph

  using component_map_t = std::unordered_map<node_t, std::size_t>;
  using comp_to_vertex_map_t =
      std::unordered_map<std::size_t, vertex_descriptor_t>;
  using comp_members_map_t =
      std::unordered_map<std::size_t, std::vector<node_t>>;
...
private:
  ...
  template <typename OrderVis> std::vector<node_t> sort() const {
    using color_map_t = std::unordered_map<node_t, boost::default_color_type>;
    color_map_t color;
    ...
    for (auto u : boost::make_iterator_range(vertices(m_g))) {
      if (get(cm, u) == boost::default_color_type::white_color)
        boost::detail::depth_first_visit_impl(m_g, u, vis, cm,
                                              boost::detail::nontruth2());
    }
...
public:
  ...
  scc_graph(G g, bool order = false /*default post-order*/)
      : m_g(g), m_same_scc_order(order),
        m_sccg(std::make_shared<scc_graph_t>()) {

    CRAB_LOG("sccg", crab::outs() << g << "\n");

    using root_map_t = std::unordered_map<node_t, node_t>;
    using color_map_t = std::unordered_map<node_t, boost::default_color_type>;
    using property_component_map_t =
        boost::associative_property_map<component_map_t>;
    ...
    boost::strong_components(
        m_g, property_component_map_t(m_comp_map),
    ...
    for (auto p : m_comp_map) {
      ...
      CRAB_LOG("sccg", crab::outs() << "Added scc graph node " << p.second
                                    << "--- id=" << v << "\n";);
    }
    ...
    for (const node_t &u : boost::make_iterator_range(vertices(m_g))) {
      for (auto e : boost::make_iterator_range(out_edges(u, m_g))) {
        ...
        auto res = add_edge(m_comp_to_vertex_map[m_comp_map[u]],
                            m_comp_to_vertex_map[m_comp_map[d]], *m_sccg);

        if (res.second)
          CRAB_LOG("sccg", crab::outs()
                               << "Added scc graph edge " << m_comp_map[u]
                               << " --> " << m_comp_map[d] << "\n");
      }
    }
