...
/* API for a ghost variable manager
   ...
   void project_and_meet(const variable_vector_t &variables,
	 		 const ghost_var_man_t &right_man,
   ...
   void rename(const variable_vector_t &old_vars, const variable_vector_t &new_vars,
               GhostDomain &val);                
...
class ghost_variable_manager_with_fixed_naming {
  ...
  using ghost_linear_constraint_system_t = typename GhostDomain::linear_constraint_system_t;  
  ...
  using ghost_reference_constraint_t = typename GhostDomain::reference_constraint_t;
  using ghost_variable_or_constant_t = typename GhostDomain::variable_or_constant_t;
  ...
  static_assert(std::is_same<variable_t, ghost_variable_t>::value,
		"Ghost variables are created by a fixed scheme so varname must have same type");
  
public:
  using ghost_variables_t = ghost_variables<Domain, GhostDomain, GhostDomainVarAlloc>;    
...
public:
  ghost_variable_manager_with_fixed_naming(get_type_fn get_type):
    m_get_type(get_type) {}
  ghost_variable_manager_with_fixed_naming(const ghost_var_manager_t &o) = default;
  ...
  boost::optional<ghost_variables_t> get(const variable_t &v) const {
    ...
    auto alloc_fn = [](GhostDomainVarAlloc &alloc,
		       const varname_t &name, const std::string &role) {
      if (role == "") {
	return name;
      } else {
  ...
  ghost_var_manager_t join(const ghost_var_manager_t &, GhostDomain &, GhostDomain &) const {
    CRAB_ERROR("This manager does not provide join operation");
  }

  ghost_var_manager_t meet(const ghost_var_manager_t &, GhostDomain &, GhostDomain &) const {
    CRAB_ERROR("This manager does not provide meet operation");
  }

  // left_val = meet(left_val, project(right_val, variables))
  void project_and_meet(const variable_vector_t &variables,
			const ghost_var_manager_t &,
			GhostDomain &left_val, const GhostDomain &right_val) {
  ...
  ghost_variables_t dup(const ghost_variables_t &gvars)  {
    ...
    auto alloc_fn = [](GhostDomainVarAlloc &alloc,
		       const ghost_varname_t &name, const std::string &role) {
...
class ghost_variable_manager_with_variable_naming {
  ...
  using number_t = typename Domain::number_t;
  using ghost_var_manager_t = ghost_variable_manager_with_variable_naming<Domain, GhostDomain, GhostDomainVarAlloc>;
...
private:  
  ...
  ghost_variables_t create_gvars(GhostDomainVarAlloc &alloc, const variable_t &v) const {
    auto alloc_fn = [](GhostDomainVarAlloc &alloc_, const varname_t, const std::string &) {
      return alloc_.next();
    };
    return ghost_variables_t::create(alloc, alloc_fn, v.name(), m_get_type(v));
  }

  ghost_variables_t create_gvars(GhostDomainVarAlloc &alloc, const ghost_variables_t &gvars) const {
    auto alloc_fn = [](GhostDomainVarAlloc &alloc_, const ghost_varname_t, const std::string &) {
      return alloc_.next();
    };
  ...
  ghost_variable_manager_with_variable_naming
  (ghost_map_t &&map, rev_ghost_map_t &&rev_map, GhostDomainVarAlloc &&alloc,
   get_type_fn get_type)
    : m_map(std::move(map)), m_rev_map(std::move(rev_map)), m_alloc(std::move(alloc)),
      m_get_type(get_type) {}
...
public:
  ...
  ghost_variable_manager_with_variable_naming(get_type_fn get_type)
    : m_get_type(get_type) {}
  ghost_variable_manager_with_variable_naming(const ghost_var_manager_t &o) = default;
  ghost_variable_manager_with_variable_naming(ghost_var_manager_t &&o) = default;  
  ghost_var_manager_t &operator=(const ghost_var_manager_t &o) = default;
  ghost_var_manager_t &operator=(ghost_var_manager_t &&o) = default;
  ...
  ghost_var_manager_t join(const ghost_var_manager_t &right_man,
    ...
    return ghost_var_manager_t(std::move(out_map),
			       std::move(out_rev_map),
			       std::move(out_alloc),
			       m_get_type);
  }
  ...
  void project_and_meet(const variable_vector_t &variables,
    ...
    for (const variable_t& v: variables) {
      if (boost::optional<ghost_variables_t> old_right_gvars = right_man.get(v)) {
  ...
  void project(const variable_vector_t &vars, GhostDomain &val) {    
    ...
    for(auto it = m_map.begin(); it != m_map.end(); ) {
      if (!std::binary_search(sorted_vars.begin(), sorted_vars.end(), it->first)) {
	it->second.remove_rev_varmap(m_rev_map);
        it = m_map.erase(it);                      
      }  else {
	++it;
  ...
  ghost_linear_constraint_t rename_linear_cst(const linear_constraint_t &cst) {
    if (cst.is_inequality() || cst.is_strict_inequality()) {
      return ghost_linear_constraint_t(rename_linear_expr(cst.expression()),
