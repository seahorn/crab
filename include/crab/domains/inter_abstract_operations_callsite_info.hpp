...
void callsite_info<Variable>::check_consistency() const {
  ...
  // each positional pair of classes must also have the same arity: a
  // misaligned grouping would otherwise be silently skipped downstream
  for (std::size_t i = 0; i < m_caller_in_groups.size(); ++i) {
    if (m_caller_in_groups[i].size() != m_callee_in_groups[i].size()) {
      CRAB_ERROR("caller and callee input region equivalent class ", i,
                 " have different sizes in callsite_info");
    }
  }
  for (std::size_t i = 0; i < m_caller_out_groups.size(); ++i) {
    if (m_caller_out_groups[i].size() != m_callee_out_groups[i].size()) {
      CRAB_ERROR("caller and callee output region equivalent class ", i,
                 " have different sizes in callsite_info");
    }
  }
}
