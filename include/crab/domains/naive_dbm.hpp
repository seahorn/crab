/*******************************************************************************
 *
 * Standard domain of Difference-Bound Matrices
 *
...
private:
  ...
  public:
    ...
    /* Resize the matrix to manage a new variable.
...
private:
  ...
  // Forget all informations about variable k
...
public:
  ...
  // Does not normalize.
  interval_t to_interval(VariableName x) const {
    if (_is_bottom) {
  ...
  bool satisfies(linear_constraint_t cst) const {
    ...
    for (typename linear_expression_t::iterator it = cst.begin();
         it != cst.end();
         ++it) {
      inv.set(it->second.name(), to_interval(it->second.name()));
    }
    inv += cst;
