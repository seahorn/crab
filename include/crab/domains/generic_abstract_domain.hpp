...
private:
  ...
  public:
    ...
    operator&&(const abstract_domain_concept &abs) const = 0;
    virtual std::unique_ptr<abstract_domain_concept>
    widening_thresholds(const abstract_domain_concept &abs,
  ...
  public:
    ...
    bool operator<=(const abstract_domain_concept &abs) const override {
      return m_inv.operator<=(static_cast<const abstract_domain_model*>(&abs)->m_inv);
    }
    ...
    operator|(const abstract_domain_concept &abs) const override {
      std::unique_ptr<abstract_domain_concept>
	res(new abstract_domain_model(
	m_inv.operator|(static_cast<const abstract_domain_model*>(&abs)->m_inv)));
      return std::move(res);
    }
    ...
    widening_thresholds(const abstract_domain_concept &abs,
      ...
      std::unique_ptr<abstract_domain_concept>
	res(new abstract_domain_model(
	m_inv.widening_thresholds(static_cast<const abstract_domain_model*>(&abs)->m_inv, ts)));
      return std::move(res);
    ...
    // unsafe: if the underlying domain in invariant is not Domain then it will crash    
    void backward_apply(arith_operation_t op, const variable_t &x,
			const variable_t &y, const variable_t &z,
			const abstract_domain_concept &invariant) override {
      m_inv.backward_apply(op, x, y, z, static_cast<const abstract_domain_model*>(&invariant)->m_inv);
    }
    // unsafe: if the underlying domain in invariant is not Domain then it will crash        
    void backward_apply(arith_operation_t op, const variable_t &x,
			const variable_t &y, number_t k,
			const abstract_domain_concept &invariant) override {
      m_inv.backward_apply(op, x, y, k, static_cast<const abstract_domain_model*>(&invariant)->m_inv);
    }
    // unsafe: if the underlying domain in invariant is not Domain then it will crash        
    void backward_assign(const variable_t &x,
			 const linear_expression_t &e,
			 const abstract_domain_concept &invariant) override {
      m_inv.backward_assign(x, e, static_cast<const abstract_domain_model*>(&invariant)->m_inv);
    }
    // unsafe: if the underlying domain in invariant is not Domain then it will crash        
    void backward_assign_bool_cst(const variable_t &lhs,
				  const linear_constraint_t &rhs,
				  const abstract_domain_concept &invariant) override {
      m_inv.backward_assign_bool_cst(lhs, rhs,
    ...
    void backward_apply_binary_bool(bool_operation_t op,
				    ...
				    const variable_t &z,
				    const abstract_domain_concept &invariant) override {
      m_inv.backward_apply_binary_bool(op, x, y, z,
				       static_cast<const abstract_domain_model*>(&invariant)->m_inv);
    }
    // unsafe: if the underlying domain in invariant is not Domain then it will crash        
    void backward_array_init(const variable_t &a,
			     ...
			     const linear_expression_t &val,
			     const abstract_domain_concept &invariant) override {
      m_inv.backward_array_init(a, elem_size, lb_idx, ub_idx, val,
				static_cast<const abstract_domain_model*>(&invariant)->m_inv);
    }
    // unsafe: if the underlying domain in invariant is not Domain then it will crash        
    void backward_array_load(const variable_t &lhs, const variable_t &a,
			     ...
			     const linear_expression_t &i,
			     const abstract_domain_concept &invariant) override {
      m_inv.backward_array_load(lhs, a, elem_size, i,
				static_cast<const abstract_domain_model*>(&invariant)->m_inv);
    }
    // unsafe: if the underlying domain in invariant is not Domain then it will crash        
    void backward_array_store(const variable_t &a,
			      ...
			      bool is_strong_update,
			      const abstract_domain_concept &invariant) override {
      m_inv.backward_array_store(a, elem_size, i, v, is_strong_update,
				 static_cast<const abstract_domain_model*>(&invariant)->m_inv);
    }
    // unsafe: if the underlying domain in invariant is not Domain then it will crash        
    void backward_array_store_range(const variable_t &a,
				    ...
				    const linear_expression_t &v,
				    const abstract_domain_concept &invariant) override {
      m_inv.backward_array_store_range(a, elem_size, i, j, v,
				       static_cast<const abstract_domain_model*>(&invariant)->m_inv);
    }
    // unsafe: if the underlying domain in invariant is not Domain then it will crash        
    void backward_array_assign(const variable_t &a, const variable_t &b,
			       const abstract_domain_concept &invariant) override {
      m_inv.backward_array_assign(a, b,
				  static_cast<const abstract_domain_model*>(&invariant)->m_inv);
    }
    ...
    void backward_intrinsic(std::string name,
			    ...
			    const variable_vector_t &outputs,
			    const abstract_domain_concept &invariant) override {
      m_inv.backward_intrinsic(name, inputs, outputs,
			       static_cast<const abstract_domain_model*>(&invariant)->m_inv);
    }
    std::string domain_name() const override {
      return m_inv.domain_name();
    }
  ...
  explicit generic_abstract_domain(std::unique_ptr<abstract_domain_concept> concept)
...
public:
  ...
  void set_to_top() override {
    m_concept->set_to_top();
  }
  void set_to_bottom() override {
    m_concept->set_to_bottom();
  }  
  bool is_bottom() const override {
    return m_concept->is_bottom();
  }
  bool is_top() const override {
    return m_concept->is_top();
  }
  ...
  generic_abstract_domain operator|(const generic_abstract_domain &abs) const override {
    return generic_abstract_domain(std::move(m_concept->operator|(*(abs.m_concept))));
  }
  generic_abstract_domain operator&(const generic_abstract_domain &abs) const override {
    return generic_abstract_domain(std::move(m_concept->operator&(*(abs.m_concept))));    
  }
  ...
  widening_thresholds(const generic_abstract_domain &abs,
                      const crab::iterators::thresholds<number_t> &ts) const override {
    return generic_abstract_domain(std::move(m_concept->widening_thresholds(*(abs.m_concept), ts)));
  }
  void apply(arith_operation_t op, const variable_t &x,
	     const variable_t &y, const variable_t &z) override {
  ...
  void backward_assign(const variable_t &x,
		       const linear_expression_t &e, const generic_abstract_domain &invariant) override {
  ...
  void backward_assign_bool_var(const variable_t &lhs,
				const variable_t &rhs, bool is_not_rhs,
				const generic_abstract_domain &invariant) override {
    m_concept->backward_assign_bool_var(lhs, rhs, is_not_rhs, *invariant.m_concept);
  }
  void backward_apply_binary_bool(bool_operation_t op,
				  const variable_t &x,
				  const variable_t &y,
  ...
  void backward_array_init(const variable_t &a,
			   ...
			   const linear_expression_t &val,
			   const generic_abstract_domain &invariant) override {
    m_concept->backward_array_init(a, elem_size, lb_idx, ub_idx, val, *invariant.m_concept);
  }
  void backward_array_load(const variable_t &lhs, const variable_t &a,
			   const linear_expression_t &elem_size,
			   const linear_expression_t &i,
  ...
  void backward_array_store(const variable_t &a,
			    ...
			    bool is_strong_update,
			    const generic_abstract_domain &invariant) override {
    m_concept->backward_array_store(a, elem_size, i, v, is_strong_update, *invariant.m_concept);
  }
  void backward_array_store_range(const variable_t &a,
				  const linear_expression_t &elem_size,
				  const linear_expression_t &i,
				  ...
				  const linear_expression_t &v,
				  const generic_abstract_domain &invariant) override {
    m_concept->backward_array_store_range(a, elem_size, i, j, v, *invariant.m_concept);
  }
  void backward_array_assign(const variable_t &a, const variable_t &b,
			     const generic_abstract_domain &invariant) override {
  ...
  std::string domain_name() const override {
    return m_concept->domain_name();
  }
  void write(crab::crab_os &o) const override {
    m_concept->write(o);
  }
  friend crab::crab_os &operator<<(crab::crab_os &o,
                                   const generic_abstract_domain<Variable> &dom) {
    dom.write(o);
    return o;
  }
};
