...
namespace crab{
  namespace domains{
    namespace ldd {
      ...
      static void Ldd_print_minterm_smtlib_aux(LddManager *ldd, LddNode *node, int* list, char** vnames);

      /**
       * Prints disjoint sum of product cover for the function in smtlibv1
       * format. Based on Ldd_PrintMinterm.
       */
      static inline int Ldd_PrintMintermSmtLibv1 (LddManager *ldd, LddNode* node, char** vnames)
      {
	int i, *list;
	// DdNode *zero;
	...
	return (1);
      ...
      static void  Ldd_print_minterm_smtlib_aux (LddManager *ldd, LddNode *n, int* list, char** vnames)
      {
	DdNode *N, *Nv, *Nnv;
	int i, v, index, p;
		...
		for (i = 0; i < CUDD->size; i++)
		  {
		    ...
		    */
		    ...
		    /* v is true */
		      ...
		      {
			THEORY->print_lincons_smtlibv1 (CUDD->out, 
							ldd->ddVars [p],
							vnames);
			fprintf (CUDD->out, " ");
		      }
