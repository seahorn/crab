...
template<typename Number>    
constant<Number> constant<Number>::bottom() { return constant(true); }

template<typename Number>      
constant<Number> constant<Number>::top() { return constant(false); }

template<typename Number>      
constant<Number> constant<Number>::zero() { return constant(Number(0));}

template<typename Number>      
bool constant<Number>::is_bottom() const { return m_is_bottom; }

template<typename Number>      
bool constant<Number>::is_top() const { return (!is_bottom() && !m_constant); }

template<typename Number>      
bool constant<Number>::is_constant() const { return m_constant != boost::none; }

template<typename Number>      
Number constant<Number>::get_constant() const {
  assert(is_constant());
  return *m_constant;
}
...
template<typename Number>  
constant<Number> constant<Number>::operator||(const constant<Number> &o) const { return *this | o; }
...
template<typename Number> 
constant<Number> constant<Number>::BitwiseLShr(const constant<Number> &o) const {
  return constant<Number>::top();
}
template<typename Number> 
constant<Number> constant<Number>::BitwiseAShr(const constant<Number> &o) const {
  return constant<Number>::top();
}
