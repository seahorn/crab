...
private:
  ...
  public:
    ...
    domain_ptr_t domain() { return _domain; }

    void set_domain(domain_ptr_t domain) { _domain = domain; }
  ...
  public:
    ...
    domain_ptr_t find_domain(const VariableName& v) {
    ...
    // Return a map root_var => list of variables in that equivalence class
    ...
    void write(std::ostream& o) {
      ...
      for (typename classes_map_t::iterator it = _classes.begin();
        ...
        domain_ptr_t domain = it->second.domain();
        o << key << " -> " << *domain;
        ++it;
        if (it != _classes.end()) {
          o << ", ";
...
private:
  ...
  // Add a relation x = f(y, z)
  domain_ptr_t add_relation(const VariableName& x,
  ...
  // Add a relation x = f(y)
  domain_ptr_t add_relation(const VariableName& x, const VariableName& y) {
  ...
  interval_t to_interval(const VariableName& x) {
    if (_is_bottom) {
      return interval_t::bottom();
    } else {
      if (_equiv_relation.contains(x)) {
        return _equiv_relation.find_domain(x)->operator[](x);
      } else {
  ...
  // Binary operation using a union semantic (join, widening)
  ...
  var_packing_domain_t union_binary_op(var_packing_domain_t& o) const {
    ...
    for (typename classes_vars_t::iterator c_it = o_classes.begin();
      ...
      domain_ptr_t o_domain = o._equiv_relation.find_domain(o_root);
  ...
  // Binary operation using an intersection semantic (meet, narrowing)
  template < typename op_t >
...
public:
  ...
  void operator-=(variable_set_t variables) {
  ...
  void apply(conv_operation_t op,
    ...
    add_relation(x, y)->apply(op, x, y, width);
    _is_normalized = false;
  }
  ...
  void apply(conv_operation_t op, VariableName x, Number k, unsigned width) {
    ...
    forget(x);
    _equiv_relation.add_equiv_class(x);
    _equiv_relation.find_domain(x)->apply(op, x, k, width);
    _is_normalized = false;
  }
