  ...
  namespace domains {
    ...
    class ArrayGrOps {
     ...
     public:
      ...
      static bool less_than (Wt w1, Wt w2) {
        return (!(w2 <= w1));
      }
      ...
      template<class G>
      static void set_edge(G& g, vert_id i, Wt w, vert_id j) {
        if (w.is_top()) return;
        g.set_edge(i, w, j);
      }

      template<class G>
      static void add_edge(G& g, vert_id i, Wt w, vert_id j) {
        if (w.is_top()) return;
        g.add_edge(i, w, j);
      }

      template<class G>
      static void apply_delta(G& g, edge_vector& delta) {
        for(auto& e : delta)
      ...
      template<class G>
      static void closure (G& g, unsigned max_cycles = 10) {
        if (IsDistWt) {
          floyd_warshall(g);
        } else {
          bool change = false;
      ...
      // XXX: we only use it for debugging
      template<class G>
      static bool floyd_warshall (G& g) {   
        bool change=false;
        ...
        for (vert_id k: g.verts ()) {
          for (vert_id i: g.verts ()) {
            for (vert_id j: g.verts ()) {
              ...
              change = maybe_extend(g, i, j, 
                                    (has_ik? (Wt) w_ik : Wt::top()), 
      ...
      // guaranteed.
      template<class G>
      static bool close_after_edge(G &g, vert_id x, vert_id y) {
