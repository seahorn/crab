...
wrapped_interval<Number> wrapped_interval<Number>::
default_implementation(const wrapped_interval<Number> &x) const {
...
template<typename Number>  
wrapped_interval<Number>::wrapped_interval(wrapint start, wrapint end, bool is_bottom)
...
template<typename Number>    
void wrapped_interval<Number>::signed_split(std::vector<wrapped_interval<Number>> &intervals) const {
  if (is_bottom())
    return;
...
template<typename Number>    
void wrapped_interval<Number>::unsigned_split(std::vector<wrapped_interval<Number>> &intervals) const {
  if (is_bottom())
    return;
...
template<typename Number>      
void wrapped_interval<Number>::signed_and_unsigned_split(std::vector<wrapped_interval<Number>> &out) const {
...
template<typename Number>      
wrapped_interval<Number> wrapped_interval<Number>::signed_mul(const wrapped_interval<Number> &x) const {
...
template<typename Number>      
wrapped_interval<Number> wrapped_interval<Number>::unsigned_mul(const wrapped_interval<Number> &x) const {
  assert(!is_bottom() && !x.is_bottom());
...
template<typename Number>      
void wrapped_interval<Number>::exact_meet(const wrapped_interval<Number> &x,
  ...
  if (is_bottom() || x.is_bottom()) {
    ...
    out.push_back(*this);
  } else if (x.at(m_start) &&
	     x.at(m_end) && at(x.m_start) && at(x.m_end)) {
    ...
    out.push_back(x);
  } else if (x.at(m_start) && at(x.m_end) &&
	     !x.at(m_end) && at(x.m_start)) {
    out.push_back(wrapped_interval<Number>(m_start, x.m_end));
  } else if (x.at(m_end) && at(x.m_start) &&
	     !x.at(m_start) && at(x.m_end)) {
    out.push_back(wrapped_interval<Number>(x.m_start, m_end));
  } else {
    // bottom
  }
...
template<typename Number>      
void wrapped_interval<Number>::reduced_signed_unsigned_mul(const wrapped_interval<Number> &x,
							   std::vector<wrapped_interval<Number>> &out) const {
  if (is_bottom() || x.is_bottom()) {
    return;
  }
...
template<typename Number>      
wrapped_interval<Number> wrapped_interval<Number>::unsigned_div(const wrapped_interval<Number> &x) const {
...
template<typename Number>      
wrapped_interval<Number> wrapped_interval<Number>::signed_div(const wrapped_interval<Number> &x) const {
  ...
  if (msb_start == msb_x_start) {
	...
	res = wrapped_interval<Number>(m_end.sdiv(x.m_start), m_start.sdiv(x.m_end));
      }
    } else { // both positive
      // -- check if division will overflow
      if (!((m_start == smin && x.m_end == minus_one) ||
	    (m_end == smin && x.m_start == minus_one))) {
	res = wrapped_interval<Number>(m_start.sdiv(x.m_end), m_end.sdiv(x.m_start));
      }
    }
  } else {
    if (msb_start) {
	...
	res = wrapped_interval<Number>(m_start.sdiv(x.m_start), m_end.sdiv(x.m_end));
      }
    } else {
      assert(msb_x_start);
      // -- check if division will overflow
	...
	res = wrapped_interval<Number>(m_end.sdiv(x.m_end), m_start.sdiv(x.m_start));
      }
    }
  }
  CRAB_LOG("wrapped-int-div", crab::outs() << res << "\n";);
...
template<typename Number>      
void wrapped_interval<Number>::trim_zero(std::vector<wrapped_interval<Number>> &out) const {
  ...
  if (!is_bottom() && (!(*this == zero))) {
    if (start() == zero) {
      ...
      out.push_back(
		    wrapped_interval<Number>(start(), wrapint(-1, get_bitwidth(__LINE__))));
    } else if (at(zero)) {
        out.push_back(
		      wrapped_interval<Number>(start(), wrapint(-1, get_bitwidth(__LINE__))));
        out.push_back(
...
template<typename Number>      
wrapped_interval<Number>::wrapped_interval(wrapint n) : m_start(n), m_end(n), m_is_bottom(false) {}
...
template<typename Number>      
wrapped_interval<Number>::wrapped_interval() : m_start(wrapint(0, 3)), m_end(7, 3), m_is_bottom(false) {}

// return top if n does not fit into a wrapint. No runtime errors.
template<typename Number>        
wrapped_interval<Number> wrapped_interval<Number>::mk_winterval(Number n, wrapint::bitwidth_t width) {
...
template<typename Number>      
wrapped_interval<Number> wrapped_interval<Number>::mk_winterval(Number lb, Number ub, wrapint::bitwidth_t width) {
...
template<typename Number>      
wrapped_interval<Number> wrapped_interval<Number>::signed_limit(wrapint::bitwidth_t b) {
...
template<typename Number>      
wrapped_interval<Number> wrapped_interval<Number>::unsigned_limit(wrapint::bitwidth_t b) {
...
template<typename Number>      
bool wrapped_interval<Number>::is_bottom() const { return m_is_bottom; }
  
template<typename Number>      
bool wrapped_interval<Number>::is_top() const {
...
template<typename Number>      
wrapped_interval<Number> wrapped_interval<Number>::lower_half_line(bool is_signed) const {
  if (is_top() || is_bottom())
    return *this;
...
template<typename Number>      
wrapped_interval<Number> wrapped_interval<Number>::upper_half_line(bool is_signed) const {
  if (is_top() || is_bottom())
    return *this;
...
template<typename Number>      
bool wrapped_interval<Number>::is_singleton() const {
...
// before than m_end.
template<typename Number>      
bool wrapped_interval<Number>::at(wrapint x) const {
  if (is_bottom()) {
    return false;
    } else if (is_top()) {
    return true;
...
template<typename Number>    
bool wrapped_interval<Number>::operator<=(const wrapped_interval<Number> &x) const {
  if (x.is_top() || is_bottom()) {
    ...
    return x.at(m_start) && x.at(m_end) &&
      (!(at(x.m_start)) || !(at(x.m_end)));
  }
}
...
template<typename Number>      
bool wrapped_interval<Number>::operator!=(const wrapped_interval<Number> &x) const { return !(this->operator==(x)); }

template<typename Number>      
wrapped_interval<Number> wrapped_interval<Number>::operator|(const wrapped_interval<Number> &x) const {
  if (*this <= x) {
    return x;
  } else if (x <= *this) {
    return *this;
...
template<typename Number>      
wrapped_interval<Number> wrapped_interval<Number>::
widening_thresholds(const wrapped_interval<Number> &x,
...
template<typename Number>        
wrapped_interval<Number> &wrapped_interval<Number>::operator+=(const wrapped_interval<Number> &x) {
...
template<typename Number>          
wrapped_interval<Number> wrapped_interval<Number>::SDiv(const wrapped_interval<Number> &x) const {
  if (is_bottom() || x.is_bottom()) {
    return wrapped_interval<Number>::bottom();
  }
  if (is_top() || x.is_top()) {
...
template<typename Number>          
wrapped_interval<Number> wrapped_interval<Number>::UDiv(const wrapped_interval<Number> &x) const {
  if (is_bottom() || x.is_bottom()) {
    return wrapped_interval<Number>::bottom();
  }
  if (is_top() || x.is_top()) {
...
template<typename Number>          
wrapped_interval<Number> wrapped_interval<Number>::SRem(const wrapped_interval<Number> &x) const {
  return default_implementation(x);
}

template<typename Number>          
wrapped_interval<Number> wrapped_interval<Number>::URem(const wrapped_interval<Number> &x) const {
  return default_implementation(x);
}
  
template<typename Number>          
wrapped_interval<Number> wrapped_interval<Number>::ZExt(unsigned bits_to_add) const {
  ...
  for (typename std::vector<wrapped_interval<Number>>::iterator
    ...
    wrapint a = (*it).start();
    wrapint b = (*it).end();
    res = res | wrapped_interval<Number>(a.zext(bits_to_add), b.zext(bits_to_add));
  }
  return res;
}
  
template<typename Number>            
wrapped_interval<Number> wrapped_interval<Number>::SExt(unsigned bits_to_add) const {
  std::vector<wrapped_interval<Number>> intervals;
  ...
  for (typename std::vector<wrapped_interval<Number>>::iterator
    ...
    wrapint a = (*it).start();
    wrapint b = (*it).end();
    res = res | wrapped_interval<Number>(a.sext(bits_to_add), b.sext(bits_to_add));
  }
  return res;
}

template<typename Number>            
wrapped_interval<Number> wrapped_interval<Number>::Trunc(unsigned bits_to_keep) const {
  if (is_bottom() || is_top()) {
    return *this;
  }
...
template<typename Number>            
wrapped_interval<Number> wrapped_interval<Number>::Shl(const wrapped_interval<Number> &x) const {
  if (is_bottom())
    return *this;
...
template<typename Number>              
wrapped_interval<Number> wrapped_interval<Number>::LShr(const wrapped_interval<Number> &x) const {
  if (is_bottom())
    return *this;
...
template<typename Number>              
wrapped_interval<Number> wrapped_interval<Number>::AShr(const wrapped_interval<Number> &x) const {
  if (is_bottom()) {
    return *this;
  }
...
template<typename Number>              
wrapped_interval<Number> wrapped_interval<Number>::And(const wrapped_interval<Number> &x) const {
  return default_implementation(x);
}
  
template<typename Number>              
wrapped_interval<Number> wrapped_interval<Number>::Or(const wrapped_interval<Number> &x) const {
  return default_implementation(x);
}
  
template<typename Number>              
wrapped_interval<Number> wrapped_interval<Number>::Xor(const wrapped_interval<Number> &x) const {
  return default_implementation(x);
}
