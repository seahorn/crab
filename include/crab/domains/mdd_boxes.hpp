...
namespace domains {
	...
	crab::ScopedCrabStats __st__(getDomainName() + ".copy");
      ...
      void set_to_top() {
	mdd_boxes_domain_t tmp(false);
	std::swap(*this, tmp);
      }

      void set_to_bottom() {
	mdd_boxes_domain_t tmp(true);
	std::swap(*this, tmp);
      }
	...
	if (is_bottom() || o.is_bottom()) {
	  return mdd_boxes_domain_t::bottom();
	} else if (is_top()) {
	  return o;
	} else if (o.is_top()) {
	  return *this;
	...
	if (op >= OP_ADDITION && op <= OP_SDIV) {		
	  ...
	  default:
	    CRAB_ERROR("mdd-boxes operation ", op, " not supported");	  
	  }
	} else {
	  ...
	  default: CRAB_ERROR("mdd-boxes operation ", op, " not supported");
	  }
	  set(x, xi);
	}
	...
	if (cst.is_tautology()) {
	  ...
	  if (cst.is_inequality()) {
	    ...
	    variable_vector_t vars_vector;
	    vars_vector.reserve(cst_vars.size());
	    std::copy(cst_vars.begin(), cst_vars.end(), std::back_inserter(vars_vector));
	    dom.project(vars_vector);
