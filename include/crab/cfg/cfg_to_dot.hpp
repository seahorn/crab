...
void cfg_body_to_dot(const CFG &cfg, const checker::checks_db &db, crab_string_os &os) {
  using basic_block_t = typename CFG::basic_block_t;
  using statement_t = typename CFG::statement_t;
  auto rewrite = [](std::string &str,
		    const std::map<std::pair<unsigned, std::string>, std::string> &rewrite_map) {
  ...
  auto statement_to_str = [&op_rewrite_map, &rewrite, &print_checks, &db]
  ...
  auto print_node = [&statement_to_str](const basic_block_t* node, crab_string_os &os){
		      os << "\tNode" << node << " ";
...
void cfg_to_dot(const CFG &cfg,
		std::function<boost::optional<Dom>(const typename CFG::basic_block_label_t&)> pre_fn,
		std::function<boost::optional<Dom>(const typename CFG::basic_block_label_t&)> post_fn,
		const checker::checks_db &db) {
  using basic_block_t = typename CFG::basic_block_t;
  auto print_invariants = [](const basic_block_t *node,
			     boost::optional<Dom> pre, boost::optional<Dom> post, crab_string_os &os) {
      if (pre) {
	os << "\tNodePreInv" << node << " ";
	os << "[style=\"filled\",fillcolor=cornsilk,label=\"" << *pre << "\"];\n";
	os << "\tNodePreInv" << node << " -> " << "Node" << node << " [arrowhead= diamond];\n";
      }
      if (post) {
	os << "\tNodePostInv" << node << " ";
	os << "[style=\"filled\",fillcolor=azure,label=\"" << *post << "\"];\n";
	os << "\tNode" << node << " -> " << "NodePostInv" << node << " [arrowhead= diamond];\n";
      }			    
   };
