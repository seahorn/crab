...
/**
 ...
 Attention: The memory of this domain is managed in a brute force
...
namespace ikos {
  ...
  private:
    ...
    private:
      ...
      vector<bound_t > matrix;	
      size_t num_var;
      
    public:
      dbmatrix(): writeable(), num_var(0) { }	// Size of 0 represents top.
      
      dbmatrix(const dbmatrix &other): writeable(), matrix(other.matrix), num_var(other.num_var) { }
      ...
      void resize(size_t new_size){	// Expected size value to represent number of contained variables.
        if(this->num_var >= new_size){	// Does not currently support downsizing.
          return;
        }
        bound_t infinite("+oo");
        this->matrix.resize(4* new_size* new_size, infinite);
        for(size_t j = 2*this->num_var; j > 0; --j){
          for(size_t i = 2*this->num_var; i > 0; --i){
            swap(this->matrix[2*new_size*(j- 1) + (i- 1)], this->matrix[2*num_var*(j- 1) + (i- 1)]);
          }
        }
      ...
      void operator-=(unsigned int k){
        ...
        size_t new_size = this->num_var - 1;
        bound_t infinite("+oo");
        vector<bound_t > new_matrix;
        ...
        for(size_t j = 2*this->num_var; j > 0; --j){
          ...
          for(size_t i = 2*this->num_var; i > 0; --i){
            ...
            if (i > 2*k){
              if (j > 2*k){
                swap(new_matrix[2*new_size*(j- 3) + (i- 3)], this->matrix[2*num_var*(j- 1) + (i- 1)]);
              }
              else{ // j < 2*k-1
                swap(new_matrix[2*new_size*(j- 1) + (i- 3)], this->matrix[2*num_var*(j- 1) + (i- 1)]);
              }
            }
            else{ // i < 2*k-1
              if (j > 2*k){
                swap(new_matrix[2*new_size*(j- 3) + (i- 1)], this->matrix[2*num_var*(j- 1) + (i- 1)]);
              }
              else{ // j < 2*k-1
                swap(new_matrix[2*new_size*(j- 1) + (i- 1)], this->matrix[2*num_var*(j- 1) + (i- 1)]);
              }
            }
          }
        }
        swap(this->matrix, new_matrix);
        this->num_var = new_size;
      }
   ...
   private:
    ...
    void apply_constraint(unsigned int var, bool is_positive, bound_t constraint){	
      ...
      if(is_positive){  // 2*v1 <= constraint
        _dbm(2*var, 2*var- 1)= bound_t::min(_dbm(2*var, 2*var- 1), constraint);
      } 
      else {	    // 2*v1 >= constraint
        _dbm(2*var- 1, 2*var)= bound_t::min(_dbm(2*var- 1, 2*var), constraint);
      }
    ...
    void apply_constraint(unsigned int i, unsigned int j, bool is1_positive, 
      ...
      if(is1_positive && is2_positive){	          // v1 + v2 <= constraint
        ...
        _dbm(2*j- 1, 2*i- 1)= bound_t::min(_dbm(2*j- 1, 2*i- 1), constraint);
        _dbm(2*i, 2*j)= bound_t::min(_dbm(2*i, 2*j), constraint);
      } 
      else if(!is1_positive && is2_positive){	// v2 - v1 <= constraint
        _dbm(2*i- 1, 2*j- 1)= bound_t::min(_dbm(2*i- 1, 2*j- 1), constraint);
  ...
  public:
    static octagon_t top() {
      return octagon(true);
    }
    
    static octagon_t bottom() {
  ...
  public:
    ...
    octagon(const octagon_t& o): 
       ...
       _dbm(o._dbm), _map(o._map), _is_normalized(o._is_normalized), 
    ...
    ostream& write(ostream& o) { 
      ...
      for(typename map_t::iterator it=_map.begin(); it!= _map.end(); ++it){
        ...
        for(typename map_t::iterator it2=it+1; it2!= _map.end(); ++it2){
          ...
          if (lb.is_finite() && rb.is_finite()){
            if (lb == rb)
              o << " " << it->first << " - " << it2->first << " = " << rb << "; ";	
            else
              o << " " << lb << " <= " << it->first << " - " << it2->first << " <= " << rb << "; ";	
          }
          else if (lb.is_finite()){
            o << " " << lb << " <= " << it->first << " - " << it2->first << "; ";	
          }
          else if (rb.is_finite()){
            o << " " << it->first << " - " << it2->first << " <= " << rb << "; ";	
          }
      ...
      { // print internal datastructures
        ...
        for(typename map_t::iterator it = _map.begin(); it != _map.end(); ++it){
          ...
          cout << it->first << "-" << " -> " << i_minus << ";";
          cout << it->first << "+" << " -> " << i_plus << ";";
        }
        cout << "}" << endl;
        cout << _dbm << endl;
      }
